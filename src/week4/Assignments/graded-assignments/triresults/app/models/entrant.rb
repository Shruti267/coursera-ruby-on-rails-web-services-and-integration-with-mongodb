class Entrant
  ...
  RESULTS.keys.each do |name|
      ...
      define_method("#{name}=") do |event|
          event=self.send("#{name}").build_event(event.attributes) # if event.respond_to? :attributes
      end
...
#  	self.secs=results.map(&:secs).
#    compact.   ## not react on results without  secs
#    inject(:+)
#  end
# update_total Version 2
  def update_total(result)
    self.secs = 0
