class Race
  ...
  has_many :entrants, foreign_key: "race._id", dependent: :delete, order: [:secs.asc, :bib.asc], inverse_of: nil
...
DEFAULT_EVENTS = {	"swim"=>{:order=>0, :name=>"swim", 	:distance=>1.0, :units=>"miles"},
  ...
  #### underneath meta defenition of methods
...
["city", "state"].each do |action|
	...
	define_method("#{action}=") do |name|
		object=self.location ||= Address.new({"#{action}" => name})
...
def self.upcoming_available_to racer
	racer_in=racer.races.map {|r| r.race.id }
	Race.upcoming.not.in(:id=>racer_in)
end #upcomm
